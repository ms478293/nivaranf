...
env_ready() {
  [[ -f "${ENV_FILE}" ]] || return 1
  ! grep -Eq '^(NEXT_PUBLIC_SUPABASE_URL|NEXT_PUBLIC_SUPABASE_ANON_KEY|SUPABASE_SERVICE_ROLE_KEY|RESEND_API_KEY|ADMIN_USERNAME|ADMIN_PASSWORD|CONTENT_PORTAL_USERNAME|CONTENT_PORTAL_PASSWORD|CONTENT_PORTAL_SESSION_SECRET)=$' "${ENV_FILE}"
}
