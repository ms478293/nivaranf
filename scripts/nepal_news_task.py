#!/usr/bin/env python3
"""Nepal_News daily pipeline for OpenClaw.

Flow:
1) Pull Nepal health/education stories from trusted RSS feeds.
2) Keep only trusted/verified candidates related to Nepal health or education.
3) Remove duplicates against prior runs + existing posts, then pick top ranked #1.
4) Apply adaptive quality threshold and cached-candidate backfill for daily continuity.
...
def build_candidate(
    ...
    nepal_hits = text_hits(merged_text, NEPAL_TERMS)
    if nepal_hits == 0:
        return None
    ...
    reason = (
        ...
        f"nepal_hits={nepal_hits}, global_hits={global_hits}"
    )
...
def validate_generated_article(payload: Dict) -> None:
    ...
    if wc < 1200:
        raise RuntimeError(f"Generated article too short ({wc} words); need >= 1200")
...
def generate_article_prompt(candidate: Candidate) -> str:
    ...
    return f"""
You are writing for Nivaran Foundation's NEPAL news desk.
...
Hard rules:
1) Topic must be Nepal-focused and related to health or education.
2) Keep Nepal-specific framing, places, and context central.
3) Article quality must be publication-ready, factual, and non-rubbish.
4) Body must be 1200-1500 words in strong paragraph form.
...
{{
  ...
  "imagePrompt": "detailed photojournalistic image prompt depicting the specific scene, people, or location from this Nepal article. Must look like a real Reuters/AP news photograph taken in Nepal. Include specific visual details from the article (e.g. Nepali location, people affected, relevant infrastructure, hospitals, schools). No abstract art, no illustrations, no text overlays, no watermarks, no logos.",
...
def write_image(repo_root: Path, image_bytes: bytes, mime_type: str, slug: str, date_str: str) -> str:
    ...
    year = date_str[:4]
    rel_path = Path("images") / "nepal-news" / year / f"{date_str}-{slug}{ext}"
...
def run_pipeline(args: argparse.Namespace) -> Dict:
    ...
    cfg = load_json(
        source_cfg_path,
        {
            "sourceUrls": [],
            "trustedDomains": [],
            "excludeTerms": ["sports", "celebrity", "entertainment", "gossip", "lottery"],
            "targetMinIn12Hours": 1,
            "hardMaxIn12Hours": 3,
            "baseQualityScore": 70,
            "forcePublishEveryRun": False,
            "minPerDay": 1,
            "maxPerDay": 3,
            "windowHours": 24,
        },
    ...
    min_per_day: int = max(1, int(cfg.get("minPerDay", 1)))
    max_per_day: int = max(min_per_day, int(cfg.get("maxPerDay", 3)))
    window_hours: int = max(12, int(cfg.get("windowHours", 24)))

    target_min = max(min_per_day, target_min)
    hard_max = max(max_per_day, hard_max)
    ...
    if (now - window_start) >= dt.timedelta(hours=window_hours):
        window_start = floor_hour(now)
        state["windowStartUtc"] = window_start.isoformat().replace("+00:00", "Z")
        state["publishedInWindow"] = 0
    ...
    hours_left = max(1, window_hours - hours_elapsed)
    must_publish_today = published_in_window < min_per_day
    publish_fallback_enabled = force_publish_every_run or must_publish_today
    ...
    if (not publish_fallback_enabled) and published_in_window >= hard_max:
        run_report["reason"] = (
            f"hard max reached in current window ({published_in_window}/{hard_max})"
        )
    ...
    if selected is None and publish_fallback_enabled and cached_candidates:
    ...
    if selected is None and publish_fallback_enabled and source_unique_candidates:
        # Mandatory publish fallback: relax title-duplicate filter but never source-link duplicates.
        selected = source_unique_candidates[0]
        fallback_used = True
        fallback_reason = "title_dedupe_relaxed"
    if selected is None and must_publish_today and candidates:
        # Last resort for mandatory daily publish if all dedupe filters eliminate options.
        selected = candidates[0]
        fallback_used = True
        fallback_reason = "mandatory_any_candidate"
    if selected is None:
        run_report["reason"] = (
            "no non-duplicate trusted Nepal health/education candidate available this run"
        )
        state.setdefault("runs", []).append(run_report)
        save_json(state_path, state)
        return run_report
    ...
    location = normalize_ws(generated.get("location", "Nepal") or "Nepal")
    ...
    main_image = write_image(
        ...
        slug=article_slug or "nepal-news",
        date_str=date_str,
    )
    ...
    article_config = {
        ...
        "distribution": ["main"],
        "commitMessage": f"Auto publish Nepal news: {title}",
    }
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Nepal_News daily pipeline")
    parser.add_argument("--repo-root", default=str(root_default), help="Repo root path")
    parser.add_argument(
        "--sources-file",
        default=str(Path(__file__).with_name("nepal-news.sources.json")),
        help="Source/trust configuration JSON",
    )
    parser.add_argument(
        "--state-file",
        default=str(root_default / ".nepal_news" / "state.json"),
        help="State file for dedupe and quota window",
    )
    parser.add_argument(
        "--tmp-dir",
        default=str(root_default / ".nepal_news" / "tmp"),
