...
if [ -f /app/scripts/global_news_task.py ]; then
...
if python3 /app/scripts/telegram_notifier.py; then
...
if [ -x /app/scripts/run_automation.sh ]; then
...
if [ -f /app/scripts/global-news.sources.json ]; then
  SOURCE_COUNT=$(python3 -c "import json; data=json.load(open('/app/scripts/global-news.sources.json')); print(len(data.get('sourceUrls', [])))")
...
if [ -x /app/scripts/setup_cron.sh ] && [ -x /app/scripts/remove_cron.sh ]; then
...
if [ -f /app/src/app/admin/news-automation/page.tsx ]; then
...
if [ -f /app/src/app/api/admin/news-automation/route.ts ]; then
...
echo ""
echo "Next steps:"
echo "1. Install cron job: bash /app/scripts/setup_cron.sh"
echo "2. Or run manually: bash /app/scripts/run_automation.sh"
