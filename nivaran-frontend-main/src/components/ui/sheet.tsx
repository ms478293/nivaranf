...
const sheetVariants = cva(
  "fixed z-50 gap-4 bg-background  transition ease-in-out data-[state=closed]:duration-300 data-[state=open]:duration-500 data-[state=open]:animate-in data-[state=closed]:animate-out",
  {
    variants: {
      side: {
        ...
        right:
          "inset-y-0 right-0 h-full w-[90%] sm:max-w-[75%]  border-l data-[state=closed]:slide-out-to-right data-[state=open]:slide-in-from-right ",
      },
    },
    defaultVariants: {
      side: "right",
...
>(
  (
    { side = "right", className, children, showCrossIcon = false, ...props },
    ref
  ) => (
    <SheetPortal>
      <SheetOverlay />
