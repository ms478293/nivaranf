...
import { addExpenseEntry } from "@/lib/api/finApi/api";
...
export function ExpenseEntryForm() {
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: async (data: z.infer<typeof entrySchema>) => {
      ...
      return await addExpenseEntry({ ...data, foundationName });
    },
    onSuccess: () => {
      console.log("Expense Entry created successfully");
      toast.success("Expense Entry created Succesfully");
      form.reset();
    },
    onError: (error: any) => {
      const errorMessage =
  ...
  return (
    <div className="max-w-lg mx-auto p-6 bg-white rounded shadow">
      <h2 className="text-xl font-bold mb-4">Create Expense Entry</h2>
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="flex justify-end">
            <Button type="submit" disabled={isPending}>
              {isPending ? "Submitting..." : "Create Expense Entry"}
            </Button>
          </div>
        </form>
      </Form>
