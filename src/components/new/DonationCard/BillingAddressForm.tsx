...
export default function BillingAddressForm({ value, onChange, searchEnabled }: { value: BillingAddress; onChange: (value: BillingAddress) => void; searchEnabled: boolean }) {
  ...
  const update = (key: keyof BillingAddress, text: string) => onChange({ ...value, [key]: text });
  useEffect(() => {
    const version = ++sequence.current;
    setSuggestions([]); setActive(-1);
    if (!searchEnabled || !typed || value.line1.trim().length < 3) { setSearching(false); return; }
    const abort = new AbortController();
    const timer = setTimeout(async () => {
      session.current ||= crypto.randomUUID();
      setSearching(true); setMessage("");
      try {
        const res = await fetch("/api/donate/address", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ query: value.line1, session: session.current }), signal: abort.signal });
  ...
  }, [value.line1, searchEnabled, typed]);

  async function choose(s: Suggestion) {
    const version = ++sequence.current;
    setSuggestions([]); setSearching(true);
    try {
      const res = await fetch("/api/donate/address", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ placeId: s.id, session: session.current }) });
      const data = await res.json();
      if (version !== sequence.current) return;
      if (!res.ok) throw new Error();
      setTyped(false);
      onChange({ ...data.address, line1: data.address.line1 || value.line1 });
  ...
  return <section className={styles.formSection} aria-labelledby="billing-heading">
    ...
    <p className={styles.sectionNote}>Use the address associated with your card.</p>
    <div className={styles.fieldGrid}>
      <div className={styles.fullField}>
        <label htmlFor="billing-line1">Street address</label>
        <div className={styles.addressSearch}>
          {searchEnabled && <Search size={16} aria-hidden="true" />}
          <input id="billing-line1" name="billing-line1" value={value.line1} autoComplete="billing address-line1" maxLength={150} placeholder={searchEnabled ? "Start typing your address…" : "House number and street"} role={searchEnabled ? "combobox" : undefined} aria-autocomplete={searchEnabled ? "list" : undefined} aria-expanded={searchEnabled ? suggestions.length > 0 : undefined} aria-controls={suggestions.length ? "billing-suggestions" : undefined} aria-activedescendant={active >= 0 ? `billing-suggestion-${active}` : undefined} onChange={(e) => { setTyped(true); update("line1", e.target.value); }} onKeyDown={(e) => {
            if (e.key === "Escape") { setSuggestions([]); setTyped(false); }
          ...
          }} onBlur={(e) => { if (!e.currentTarget.parentElement?.contains(e.relatedTarget as Node)) setSuggestions([]); }} />
        ...
        <p className={styles.fieldHelp} aria-live="polite">{searching ? "Finding your address…" : message || (searchEnabled ? "Worldwide address search. You can also type your address manually." : "Your browser can fill a saved address, or you can enter it below.")}</p>
      </div>
