...
import { getFinsByIdForWeb } from "@/lib/api/finApi/api";
import { getImpactForWeb } from "@/lib/api/impactApi/api";
                  ...
                  <TableCell className="text-right text-nowrap">
                    {
                      ...
                      new Date(entry.updatedAt?.toString()).toDateString()
                    }
                  </TableCell>
                </TableRow>
              );
...
const ImpactCard: FC<{ impact: ImpactReport }> = ({ impact }) => {
  ...
  const { data: financialReportData } = useQuery({
    ...
    queryFn: () => {
      if (impact.financialReportId) {
        return getFinsByIdForWeb(impact.financialReportId);
      }
  ...
  return (
    <div>
      ...
      <div className={show ? "border rounded-lg  p-4 my-4" : "hidden"}>
...
export function ViewImpactReportsWeb() {
  const {
    data: reports,
    isLoading,
    isError,
    ...
    queryKey: ["impacts"],
    queryFn: () => getImpactForWeb({ getByFoundation: true }),
  });
